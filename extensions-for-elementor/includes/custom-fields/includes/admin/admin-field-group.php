	...
	class acf_admin_field_group {
		...
		*/
		...
		function screen_settings( $html ) {
		    ...
		    ob_start();
		    ?>
		    <div id="acf-append-show-on-screen" class="acf-hidden">
		        <label for="acf-field-key-hide">
		            <input id="acf-field-key-hide" type="checkbox" value="1" name="show_field_keys" <?php echo $checked; ?> /> 
		            <?php esc_html_e( 'Field Keys', 'acf' ); ?>
		        </label>
		    </div>
		    <?php
		    $output = ob_get_clean();
		...
		*/
		...
		function ajax_move_field() {
			...
			if ( $args['field_id'] && $args['field_group_id'] ) {
				...
				// Output HTML.
                ?>
                    <p><strong><?php echo __( 'Move Complete.', 'acf' ); ?></strong></p>
                    <p>The <?php echo esc_html( $field['label'] ); ?> field can now be found in the <a href="<?php echo esc_attr(admin_url( 'post.php?post=' . $field_group['ID'] . '&action=edit' )); ?>" target="_blank"><?php echo esc_html( $field_group['title'] ); ?></a> field group</p>
                    <a href="#" class="button button-primary acf-close-popup"><?php echo __( 'Close Window', 'acf' ); ?></a>
                <?php
				die();
			}
			...
			?>
			    <p><?php echo __( 'Please select the destination for this field', 'acf' ); ?></p>
            <?php

            ?>
			    <form id="acf-move-field-form">
            <?php

				// render
				acf_render_field_wrap( $field );
            ?>
                <button type="submit" class="button button-primary"><?php echo __( 'Move Field', 'acf' ); ?></button>
                </form>
            <?php
