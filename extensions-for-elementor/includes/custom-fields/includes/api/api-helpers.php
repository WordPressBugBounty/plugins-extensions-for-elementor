...
function acf_nonce_input( $nonce = '' ) {
    ...
    ?>
    <input type="hidden" name="_acf_nonce" value="<?php echo esc_attr( $nonce_value ); ?>" />
    <?php
}
...
function acf_debug() {
    ...
    $script_content = 'console.log("' . esc_js($s) . '"';
    if (!empty($args)) {
        foreach ($args as $arg) {
    ...
    // echo script
    ?>
    <script type="text/javascript"><?php echo esc_html($nl . $script_content . $nl); ?></script>;
    <?php
}
