...
function acf_render_field_wrap( $field, $element = 'div', $instruction = 'label' ) {
	...
	// Render HTML
    ?>
	<<?php echo esc_attr($element); ?> <?php echo esc_attr($attributes_html); ?>>"
    <?php
	if ( $element !== 'td' ) {
	    ?>
		<<?php echo esc_attr($inner_element); ?> class="acf-label">
        <?php
		...
		?>
			</<?php echo esc_attr($inner_element); ?>>
        <?php
	}
	?>
		<<?php echo esc_attr($inner_element); ?> class="acf-input">
        <?php
...
function acf_render_field_label( $field ) {
	...
	if ( $label ) {
	    ?>
	    <label <?php echo $field['id'] ? 'for="' . esc_attr( $field['id'] ) . '"' : ''; ?>><?php echo acf_esc_html( $label ); ?></label>
	    <?php
	}
