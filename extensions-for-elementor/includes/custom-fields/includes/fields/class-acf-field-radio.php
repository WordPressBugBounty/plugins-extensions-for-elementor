	...
	class acf_field_radio extends acf_field {
		...
		*/
		...
		function render_field( $field ) {
			...
			// Hiden input.
			echo acf_get_hidden_input( array( 'name' => $field['name'] ) );

			// Open <ul>.
            ?>
			<ul <?php echo acf_esc_attr( $ul ); ?>>
			...
			foreach ( $field['choices'] as $value => $label ) {
				...
				// append
                ?>
				<li><label<?php echo esc_attr(( $is_selected ? ' class="selected"' : '' )); ?>><input <?php echo acf_esc_attr( $attrs ); ?>/><?php echo acf_esc_html( $label ); ?></label>
                    <?php if ( $value === 'other' && $other_input ) {
                        echo acf_get_text_input( $other_input );
                    } ?>
                </li>
                <?php
			}
